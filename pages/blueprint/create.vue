...
const submit = async () => {
  ...
  if (totalGrade !== maximumGrade) {
    useAppToaster().show('danger', t('total-grade-must-equal-maximum-grade'))
  }
  try {
...
<template>
  <div class="mb-2 rounded-3xl bg-white p-3">
    <AppInputField
      ...
      :placeholder="$t('wirting-title')"
      :label="$t('title')"
    />
    <div class="my-2 grid grid-cols-3 gap-5">
      <AppInputField
        v-model="body.successGrade.$model"
        :errors="body.successGrade.$errors"
        :placeholder="$t('wirting-grade')"
        :label="$t('success-grade')"
      />
      <AppInputField
        v-model="body.halfSuccessGrade.$model"
        :errors="body.halfSuccessGrade.$errors"
        :placeholder="$t('wirting-grade')"
        :label="$t('half-success-grade')"
      />
      <AppInputField
        v-model="body.maximumGrade.$model"
        :errors="body.maximumGrade.$errors"
        :placeholder="$t('maximum-grade')"
    ...
    <AppAutoCompleteField
      ...
      :errors="body.questionBanks.$errors"
    ...
    <BaseCard
      v-for="(questionBank, questionBankIndex) in body.questionBanks.$model"
    ...
    >
      ...
      <AppTable
        ...
        :items="body.questionBanks.$model[questionBankIndex].topics"
        class="mt-3"
      >
        <template #data-topicId="{ index }">
          <AppAutoCompleteField
            v-model="body.questionBanks.$model[questionBankIndex].topics[index].topicId"
            :items="topicsByQuestionBankId(questionBank.questionBankId)"
            item-label="name"
            item-value="id"
          />
        </template>
        <template #data-questionType="{ index }">
          <AppAutoCompleteField
            v-model="body.questionBanks.$model[questionBankIndex].topics[index].questionType"
            :items="
              questionTypeOptions($t).filter((option) =>
        ...
        <template #data-grade="{ index }">
          <AppInputField
            v-model="body.questionBanks.$model[questionBankIndex].topics[index].grade"
            type="number"
            required
            :label="$t('grade')"
          />
      ...
      <div class="flex justify-end">
        <BaseButton
          ...
          class="gap-1"
          variant="pastel"
          @click="addTopic(questionBankIndex)"
        >
    ...
    <div class="my-3 grid gap-5 md:grid-cols-6">
      <BaseTag
        :color="body.displayResult.$model ? 'primary' : 'muted'"
        variant="pastel"
        size="lg"
      ...
      <BaseTag
        :color="body.moveBetweenQuestion.$model ? 'primary' : 'muted'"
        variant="pastel"
        size="lg"
        class="pa-3 flex cursor-pointer items-center gap-2 text-lg"
      ...
      <BaseTag
        :color="body.randomizeAnswer.$model ? 'primary' : 'muted'"
        variant="pastel"
        size="lg"
        class="pa-3 flex cursor-pointer items-center gap-2 text-lg"
      ...
      <BaseTag
        :color="body.randomizeChoices.$model ? 'primary' : 'muted'"
        variant="pastel"
        size="lg"
        class="pa-3 flex cursor-pointer items-center gap-2 text-lg"
