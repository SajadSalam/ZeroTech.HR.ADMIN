...
<template>
  <div>
    <AppCrud
v-model:current-page="filters.pageNumber" pagination :total-pages="questionBankStore.totalPages"
...
v-model="filters.subjectId" fetch-on-search search-key="name" :placeholder="$t('subject')"
          get-url="/subjects" item-label="name" item-value="id" />
        <AppAutoCompleteField
v-model="filters.topics" fetch-on-search search-key="name" :placeholder="$t('topics')"
          get-url="/topics" item-label="name" item-value="id" multiple />
      </template>
      <AppTable
v-if="viewType === ViewType.Table" :title="$t('questions-bank')" :headers="tableHeader($t)"
        :items="questionBanks">
        ...
        <template #data-actions="{ item }">
          <div class="flex items-center justify-center gap-2">
            <BaseButton
v-if="hasPrivilege('ums:ems:question-bank:view')" :data-nui-tooltip="$t('edit-questions')"
              :to="`/question-bank/questions/${item.id}`" size="sm" color="primary">
...
v-if="
        ...
        <template #data-auditor="{ item }">
          <BaseButton
v-if="hasPrivilege('ums:ems:question-bank:assign-auditors')" color="primary" variant="outline"
            @click="openAssignAuditor(item)">
            <Icon name="ph-eye" />
            {{ $t('view-auditor') }}
          </BaseButton>
        </template>
        ...
        <template #data-creator="{ item }">
          <BaseButton
v-if="hasPrivilege('ums:ems:question-bank:assign-creators')" color="primary" variant="outline"
            @click="openAssignCreator(item)">
            <Icon name="ph-eye" />
            {{ $t('view-creator') }}
          </BaseButton>
        </template>
      ...
      <div v-else class="grid gap-2 md:grid-cols-3">
        <CardView
v-for="questionBank in questionBanks" :key="questionBank.id" :question-bank="questionBank"
