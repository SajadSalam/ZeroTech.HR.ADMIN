...
<template>
    <div class="border-b-1 relative mb-5 flex h-20 items-center gap-2 bg-white"
        :class="props.horizontalScroll && 'px-4 xl:px-10'">
        <TairoCollapseBurger v-if="showNavBurger" class="ms-3" />

        <div>
            <BaseHeading v-if="app.tairo?.collapse?.toolbar?.showTitle" as="h1" size="lg"
                class="hidden text-muted-800 dark:text-white md:block">
                <slot name="title">
                    {{ route.meta.title ?? 'home-page' }}
                </slot>
            </BaseHeading>
            <span class="text-sm text-muted-500">
                {{ route.meta.description ?? 'this-is-the-home-page' }}
            </span>
        </div>
