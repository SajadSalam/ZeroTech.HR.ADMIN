...
<template>
  <BaseCard
    class="pointer-events-auto relative z-[200] flex border bg-white shadow-md dark:bg-muted-900"
  ...
  >
    ...
    <div
      ...
      :class="iconBgClasses"
    >
      <Icon :name="props.icon" class="size-6" :class="iconClasses" />
    </div>
    <div class="text-md min-w-[220px] max-w-xl py-4 pe-12 ps-8">
      <strong v-if="props.title" class="text-semibold maw-w-lg block truncate font-heading">
        {{ props.title }}
      </strong>
      <p v-if="props.message" class="whitespace-pre-wrap text-muted-600 dark:text-muted-400">
        {{ props.message }}
      </p>
    </div>
  </BaseCard>
