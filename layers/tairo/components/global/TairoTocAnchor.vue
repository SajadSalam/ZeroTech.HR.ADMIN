...
<template>
  <NuxtLink
    ...
    class="tairo-toc-anchor group/toc nui-focus relative"
    :style="{ scrollMarginTop: '1.5rem' }"
  >
    <span v-if="props.prefix || 'prefix' in $slots" :class="props.prefixClasses">
      <slot name="prefix">{{ props.prefix }}</slot>
    </span>
    <span>{{ props.label }}</span>
    <span v-if="props.suffix || 'suffix' in $slots" :class="props.suffixClasses">
