...
const hasMinLength = computed(() => props.value.length >= props.minLength ?? false)
const hasGoodLength = computed(() => props.value.length >= props.goodLength ?? false)
...
<template>
  <div class="group/password-strength relative cursor-pointer">
    <div
      class="pointer-events-none absolute left-0 top-4 z-20 rounded-lg border border-muted-200 bg-white p-6 opacity-0 shadow-xl shadow-muted-300/30 transition-opacity duration-300 group-hover/password-strength:pointer-events-auto group-hover/password-strength:opacity-100 dark:border-muted-700 dark:bg-muted-800 dark:shadow-muted-800/20"
    >
        ...
        >
          ...
          <span class="w-7">
            <Icon
              ...
              class="size-4 text-success-400 dark:text-success-500"
            />
            <Icon
              v-else
              name="ion:close-circle-outline"
              class="size-4 text-danger-400 dark:text-danger-500"
            />
          </span>
        </li>
      </ul>
