...
> {
  ...
  onError?: (error: any, ctx: MultiStepFormContext<DATA, META>) => Promise<void> | void
}
...
>(rules: MultiStepFormConfig<DATA, META>): MultiStepFormContext<DATA, META> {
  ...
  const currentStepId = computed(
    () => steps.value.find((step) => step.to === router.currentRoute.value.path)?.id ?? 0
  )
  ...
  async function validateStep(step = currentStep.value) {
    if (step.validate) {
      try {
        ...
        const hasErrors =
          Object.values(errors.value.fields).filter(Boolean).length > 0 || errors.value.message
...
export function useMultiStepForm<
  ...
  META extends Record<string, any> = Record<string, any>,
>() {
  const context = inject(injectionKey as InjectionKey<MultiStepFormContext<DATA, META>>, null)
  if (!context) {
    throw new Error('MultiStepForm: no context found, did you forget to call provideMultiStepForm?')
  }
