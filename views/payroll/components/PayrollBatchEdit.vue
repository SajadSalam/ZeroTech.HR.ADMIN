...
import type { PayrollBatchUpdateDto } from '../types'

const payrollStore = usePayrollStore()

const validator = new Validator<PayrollBatchUpdateDto>(
  {
    id: 0,
    title: '',
    periodStartDate: '',
...
const updatePayrollBatch = async () => {
  ...
  await payrollStore.updatePayrollBatch(validator.extractBody())
  payrollStore.isEditDialogOpen = false
}

watch(
  () => payrollStore.isEditDialogOpen,
  (val: boolean) => {
    if (val && payrollStore.selectedBatch) {
      const batch = payrollStore.selectedBatch
      validator.setBody({
        id: batch.id,
        title: batch.title,
        periodStartDate: batch.periodStartDate.split('T')[0], // Extract date part
        periodEndDate: batch.periodEndDate.split('T')[0], // Extract date part
        notes: batch.notes || '',
      })
    }
  }
...
<template>
  <AppDialog
    v-model="payrollStore.isEditDialogOpen"
  ...
  >
    ...
    <template #actions>
      <BaseButton color="primary" class="gap-1" :loading="isLoading" @click="updatePayrollBatch">
