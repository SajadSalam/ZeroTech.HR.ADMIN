...
const createGroup = async () => {
  ...
  validator.resetBody()
  groupStore.isCreateDialogOpen = false
}

watch(
  () => groupStore.isCreateDialogOpen,
  (val: boolean) => {
    if (val) {
      validator.resetBody()
      selectedOrgs.value = []
...
<template>
  <AppDialog
    v-model="groupStore.isCreateDialogOpen"
  ...
  >
    ...
    <template #actions>
      <BaseButton color="primary" class="gap-1" :loading="isLoading" @click="createGroup">
