...
import type { LocationTimestampDto, MapZoneDisplay, EmployeesLocation } from '../types'
...
const { updateMarkers, clearMarkers } = useEmployeeMarkers(map, avatarCache, {
    onMarkerClick: (point) => navigateTo(`/tracking/${point.employeeId}`),
})
...
const employees = ref<EmployeesLocation[]>([])
...
const loadEmployeeLocations = async () => {
    ...
    try {
        ...
        await attendanceStore.getEmployeesLocations()
        employees.value = attendanceStore.employeesLocations
...
<template>
    <div class="relative">
        ...
        <!-- Map Container -->
        <div ref="mapContainer" class="w-full rounded-2xl border border-muted-200 dark:border-muted-700 overflow-hidden"
        ...
        <div v-if="!map"
            ...
            <div class="text-center">
                <div
                    class="animate-spin rounded-full h-10 w-10 border-4 border-primary-600 border-t-transparent mx-auto mb-3">
                </div>
        ...
        <!-- Empty State -->
        <div v-if="map && !isLoadingData && employees.length === 0"
            class="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div
                class="bg-white dark:bg-muted-800 rounded-2xl p-6 shadow-lg border border-muted-200 dark:border-muted-600 text-center pointer-events-auto">
