...
const onEdit = (currentItem: Item) => {
  if (props.editButtonAction) props.editButtonAction(currentItem.documentId)
}
const onDelete = (currentItem: Item) => {
  if (props.deleteService) {
    appCrudStore.setDeleteService(() => props.deleteService!(currentItem.documentId))
  }
  appCrudStore.setItemId(currentItem.documentId)
