...
<template>
  <div class="flex items-center gap-3 mx-2">
    ...
    <div class="flex flex-col">
      <span class="text-sm font-bold">{{ useAppUserStore().user.fullName }}</span>
      <span class="text-xs font-bold text-muted-500">{{ getEnumByKey(UserRoles, 'name',
        useAppUserStore().user.role).arName }}</span>
    </div>
    <div class="flex">
      <BaseDropdown variant="text">
        ...
        <BaseDropdownItem title="تسجيل خروج" rounded="sm" @click="useAppUserStore().logout()" />
      </BaseDropdown>
    </div>
  </div>
  <ChangeUserPassword />
